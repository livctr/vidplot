...
class MovingSquareStreamer(DataStreamer):
    def __init__(self, name, sample_rate=30.0, duration=5.0, frame_size=(64, 64), square_size=16):
...
class ColoredSquareRenderer(Renderer):
    def __init__(self, name, data_streamer, grid_row, grid_column, color=(255,0,0), z_index=0):
...
def test_video_30fps():
    ...
    outpath = str(tmp_path / 'video_30fps.mp4')
    orch.show_layout(str(tmp_path / 'layout_30fps.png'))
...
def test_video_1fps():
    ...
    orch.set_annotators([streamer], [renderer], [('sq', 'r')])
    outpath = str(tmp_path / 'video_1fps.mp4')
    orch.show_layout(str(tmp_path / 'layout_1fps.png'))
...
def test_video_side_by_side_diff_fps():
    ...
    orch.set_annotators([streamer1, streamer2], [renderer1, renderer2], [('sq1', 'r1'), ('sq2', 'r2')])
    outpath = str(tmp_path / 'video_side_by_side_diff_fps.mp4')
    orch.show_layout(str(tmp_path / 'layout_side_by_side_diff_fps.png'))
    orch.write(outpath, fps=30.0)
    assert os.path.exists(outpath) 
...
def test_z_index_functionality():
    ...
    red_renderer = ColoredSquareRenderer('red', streamer1, (1,1), (1,1), color=(0,0,255), z_index=0)
    blue_renderer = ColoredSquareRenderer('blue', streamer2, (1,1), (1,1), color=(255,0,0), z_index=1)
    
    orch = AnnotationOrchestrator([64], [64])
    orch.set_annotators([streamer1, streamer2], [red_renderer, blue_renderer], 
                       [('sq1', 'red'), ('sq2', 'blue')])
    
    outpath = str(tmp_path / 'video_z_index_test.mp4')
    orch.show_layout(str(tmp_path / 'layout_z_index_test.png'))
    orch.write(outpath, fps=30.0)
    assert os.path.exists(outpath)
...
def test_different_duration_streamers():
    ...
    orch.set_annotators([short_streamer, long_streamer], [renderer1, renderer2], 
                       [('short', 'r1'), ('long', 'r2')])
    
    outpath = str(tmp_path / 'video_different_durations.mp4')
    orch.show_layout(str(tmp_path / 'layout_different_durations.png'))
    orch.write(outpath, fps=30.0)
...
def test_duplicate_names():
    ...
    orch.set_annotators([streamer1, streamer2], [renderer1, renderer2], 
                       [('sq', 'r'), ('sq', 'r')])
